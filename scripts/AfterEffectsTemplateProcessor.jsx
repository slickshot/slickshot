...
function processJob(job) {
	...
	GLOBALS.job = job;
	GLOBALS.jobFolder = GLOBALS.dropboxFolder + "\\" + job + "\\";
	var currentFolder = new Folder(GLOBALS.jobFolder);
	var jobCSV = new File(GLOBALS.jobFolder + job + ".csv");
...
function markJobAsComplete(job) {
    var file = new File(GLOBALS.dropboxFolder + "processed.txt");
    file.open("a");
    file.writeln(job);
    file.close();
}
...
function main() {
	// Check for new jobs
	var indexed = readFileIntoArray(GLOBALS.dropboxFolder + "index.txt"),
		processed = readFileIntoArray(GLOBALS.dropboxFolder + "processed.txt"),
		jobs = _.difference(indexed, processed);

	if (!jobs.length) {
		$.writeln("Scheduling main() for 60 seconds time.")
		app.scheduleTask("main()", 60000, false);
	} else {
		// Process new jobs
		for (var i = 0; i < jobs.length; i++) {
			$.writeln("Processing " + jobs[i])
			processJob(jobs[i]);

			$.writeln("Completed " + jobs[i])
			markJobAsComplete(jobs[i]);
		}
		// Check again in a few seconds
		$.writeln("Scheduling main() for 5 seconds time.")
		app.scheduleTask("main()", 5000, false);
	}
}



function randomCompGeneration(itemsArray){
	...
	for (var h = 0; h < groupOrder.length; h++){
		if (groupOrder[h] == 1){
			...
			assembleNewComp(itemsArray, 1, templateRegEx);
		}

		if (groupOrder[h] == 2){
			var templateRegEx = new RegExp(/_Template02$/);
			assembleNewComp(itemsArray, 2, templateRegEx);
		}

		if (groupOrder[h] == 3){
			var templateRegEx = new RegExp(/_Template03$/);
			assembleNewComp(itemsArray, 3, templateRegEx);
		}
...
function assembleNewComp(itemsArray, numImages, templateNeeded){
	...
	if (foundTemplate){
        ...
        for (var c = 1; c <= numImages; c++){
	        var io = new ImportOptions(File(GLOBALS.jobFolder + itemsArray[imagesAdded][0]));
